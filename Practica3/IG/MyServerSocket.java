...
public class MyServerSocket extends ServerSocket {
    ...
    public void execute(JTextArea msg, DefaultListModel<String> lm, ConcurrentHashMap<String, String> writen) throws IOException {
		while (true) {
			select.select();
			...
			while (iterator.hasNext()) {
				...
				iterator.remove();
				connect(key, msg, lm, writen);
			}
			select.selectedKeys().clear();
		}
	}
	
	private void connect(SelectionKey key, JTextArea msg, DefaultListModel<String> lm, ConcurrentHashMap<String, String> writen) throws IOException {
